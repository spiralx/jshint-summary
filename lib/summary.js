...
exports.reporter = function(result, data, options) {
  ...
  var brush = utils.getBrushes(options);
  ...
  result.forEach(function(item) {
    if (item.file !== prevfile) {
      ...
      lines.push('');
      lines.push(brush.file(utils.underline(item.file)));
  ...
  if (options.statistics) {
    data.forEach(function(data) {
      if (data.implieds || data.unused || data.globals) {
        lines.push('');
        lines.push(brush.file(utils.underline(data.file)));
